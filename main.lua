...
function love.load(arg)
	...
	--AUDIO--
	--sounds
	soundstoload = {"jump", "jumpbig", "stomp", "shot", "blockhit", "blockbreak", "coin", "decacoin", "pipe", "boom", "mushroomappear", "mushroomeat", "shrink", "death", "gameover", "fireball",
...
function love.update(dt)
	...
	while steptimer >= targetdt do
		...
		love.window.setTitle("FPS:" .. love.timer.getFPS() .. " - Send feedback/mappacks to frostygasm@gmail.com")
	end
end
...
function openSaveFolder(subfolder) --By Slime
	...
	if os.getenv("WINDIR") then -- lolwindows
		...
		if path:match("LOVE") then --hardcoded to fix ISO characters in usernames and made sure release mode doesn't mess anything up -saso
			cmdstr = "Explorer %%appdata%%\\LOVE\\mari0_fe"
		else
			cmdstr = "Explorer %%appdata%%\\mari0_fe"
		end
		path = path:gsub("/", "\\")
		successval = 1
